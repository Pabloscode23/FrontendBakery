...
const Register = () => {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            const response = await fetch("https://3eb9444quf.execute-api.us-east-2.amazonaws.com/prod/users", {
                method: "POST",
                headers: {
        ...
        <div className="flex justify-center items-center bg-[url('src/assets/img/background.jpg')] bg-cover bg-center min-h-screen px-4">
            <div className="py-7 bg-white p-6 md:p-10 rounded-lg shadow-lg w-11/12 max-w-md mt-20 lg:mt-10 mb-10">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="mb-4">
                        <label htmlFor="confirmPassword" className="block text-sm font-medium text-gray-600">Confirmar Contrase�a</label>
                        <input
                            id="confirmPassword"
                            type="password"
                    ...
                    <button type="submit" className="w-full py-2 cursor-pointer bg-[var(--color-brown-middle)] hover:bg-[var(--color-brown-dark)] text-white rounded-md">
                        Registrarse
                    </button>
                </form>

                <p className="mt-4 text-center text-sm text-gray-600">
                    �Ya tienes cuenta? <Link to="/pages/Login" className="text-indigo-500 hover:text-indigo-700">Iniciar sesi�n</Link>
                </p>
            </div>
        </div>
    );
