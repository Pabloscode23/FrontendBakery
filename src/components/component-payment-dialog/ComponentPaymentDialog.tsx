...
const LocationMarker = ({
  ...
  setLocation: (pos: [number, number]) => void;
}) => {
  useMapEvents({
    click(e) {
...
const ComponentPaymentDialog: React.FC<Props> = ({ onClose }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      address = {
        street:
          addressData?.road || addressData?.pedestrian || "Calle desconocida",
        city:
          addressData?.city ||
          addressData?.town ||
          addressData?.village ||
      ...
      console.warn(
        "No se pudo obtener la dirección exacta. Usando valores por defecto."
      );
    }
